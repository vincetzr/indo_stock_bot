...
| 14 | chart vision | **PARTIAL** | `paint_suite.py` renders; no vision read-back. H61 measured the base rates a read-back would need: 6 classic patterns, all NEGATIVE against a matched control in both halves, predicted null fired at z −47.65 |
...
| 22–25 | multibagger | **PARTIAL** | H23/H24 base rates + `multibagger.py`, a THESIS-rung calculator; the PREDICTION-rung engine stays unbuildable (H57: no fundamentals; H23: effective n ~6/decile) |
...
| ALFRED vintages | **timeout** on `alfred.stlouisfed.org`; **the api host is 400 (key missing), i.e. REACHABLE** | JSON | full US macro **with vintages** | daily | high | free | **free FRED key needed — user must register** | **RE-PROBED 2026-09-07 and the earlier row was misleading.** `fred.stlouisfed.org` and `alfred.stlouisfed.org` both time out, so the keyless CSV route is genuinely blocked. But **ALFRED vintages do not need the ALFRED host at all** — `api.stlouisfed.org/fred/series/observations` takes `realtime_start`/`realtime_end`, which IS the vintage mechanism, and that host answers. So §5-§6 are gated on **one free key**, not on two unreachable hosts. This is the A12 shape again: a true statement about one HOST read as a fact about the DATA. |
| World Bank indicators | **200**, keyless, 15 KB for Indonesia CPI | JSON | annual macro for IDN | annual | medium | free | **none** | the one macro source that works today with no key; **annual only, and revised-only — no vintages**, so it is descriptive |
| BPS webapi | **200** with `{"status":"Error","message":"Parameter Key is Missing."}` | JSON | Indonesian official statistics | monthly | high | free | **free key needed — user must register** | reachable, so this is a registration step and not a data question |
| GDELT doc API | **200** then **429**; re-probed 2026-09-07 | JSON | per-entity daily volume 2015→; one request spans an arbitrary date range | daily | **rate-limited, sustainable rate NOT established** (6s→429, 15s→2 of 4, 20s→still failed) | free | none | raw 15-min archive at `data.gdeltproject.org/gdeltv2/` — `masterfilelist.txt` is 200 and 127 MB listing ~150,000 immutable files; the corpus is terabytes. **Recorded as available and not taken.** BigQuery also on the free tier |
...
| 4 | **FRED + BPS keys** | re-probed 2026-09-07: **ALFRED vintages are served by `api.stlouisfed.org` via `realtime_start`/`realtime_end`, not by the ALFRED host**, so this is ONE registration step and not two unreachable hosts | vintages retrievable through the keyed API | revised-only series ⇒ macro stays descriptive; World Bank works keyless but is **annual and revised-only** | reproduce a known revision | 1 h once the key exists, **needs user to register two free keys** |
| 5 | ~~**Regime engine (§7)**~~ **DONE (H60).** `scripts/regime.py`. HDBSCAN returns **0 clusters, 100% noise** at any minimum size worth calling a regime — A10's warning confirmed on market state. GMM forced to k shows a screen-edge inversion at k=3 (z +4.03) that resolves to **one window, 2008-09→2009-04, at every k where the sign flips** | — | 13 episodes at k=2 is the sample; dropping the GFC episode halves the spread; no cost model | rotation null (not a shuffle) + drop-largest-episode + every k reported | done |
| 6 | ~~**Sector rotation (§8)**~~ **DONE (H59).** `scripts/sector.py`. The frozen-map survivorship worry does NOT bite — **97.1% of dead names carry a sector**. Sector momentum is worth **+5.05%/yr against a random sector choice at matched concentration** and LESS than the diversification spent to exploit it | — | rank IC z +1.50, under-powered (114 marks needed, 75 observed); IDX-IC is a backward projection before 2021 | whole-date-reassignment null + a random-sector control at matched count | done |
| 7 | **GDELT narrative collection (§18–21)** — CLIENT BUILT (`src/idxbot/data/gdelt.py`), COLLECTION NOT SCHEDULED | the unit price is far better than assumed: `startdatetime`/`enddatetime` span arbitrary ranges, so a full history for one name is **ONE request**, ~840 for the universe — not one per name-day | GDELT's index is stable for past dates | **UNVERIFIED and that is the blocker.** `timelinevol` queries the CURRENT index, which GDELT has reprocessed and expanded; if it retro-indexes, a narrative feature is look-ahead with no visible symptom. Also: **the sustainable rate is not established** — 6s gets 429, 15s succeeded 2 of 4, 20s still failed, and probing harder means hammering a rate-limited public API to see how hard you can hammer it. And the A12 relevance problem is WORSE here than in RSS: `GULA` returns **238 of 364 non-zero days** in 2023, a dense series about sugar | `gdelt.stability()` compares two fetches of the same window; every cached row carries `fetched_at` so the comparison stays possible. Quarantined by an AST test until it comes back clean | client done; collection blocked |
| 8 | ~~**Multibagger scenario model (§24)**~~ **BUILT AS A CALCULATOR THAT REFUSES TO PREDICT** (`src/idxbot/multibagger.py`). Stage 3 came back negative, so the engine §24 asks for is unbuildable — and even with fundamentals, H23's effective n of **56 whole-panel / ~6 per decile** and **30 distinct names ever in the top decile** could not validate a name-level claim | — | produces no probability for any scenario, by construction: a `Value` with rung PREDICTION raises | scenario and base rate are printed side by side and **never multiplied**; every rate carries its horizon and effective n; the stored base rates are asserted equal to `decade.BY_HORIZON` | done |
| 9 | ~~**Chart vision (§14)**~~ **THE PREREQUISITE IS BUILT; THE VISION LAYER IS NOT** (`scripts/patterns.py`, H61). No vision endpoint is callable from this process, and a layer returning "double bottom, high confidence" is unusable until someone has measured what a double bottom is worth. Six classic patterns, causal by construction, scored against a control matched on (ticker, year) and count | — | **every one is NEGATIVE against its control in both halves**, and the PREDICTED NULL `digit` fired at z −47.65, so significance in this table is not evidence | causality test at three truncation dates + a deliberately-peeking positive control; planted-shape controls; 0 of 6 beat the control after the fee alone | done |
