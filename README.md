...
| **Cross-sectional momentum book beats the universe** | ✅ **VALIDATED, and audited for outliers.** Walk-forward 5 folds: mean OOS CAGR +31.7% vs +5.3% equal-weight (+26.4% excess), 5/5 positive. Survives capping forward returns at +100% (+23.2% excess, 5/5) and 99th-percentile winsorising (+16.8%, 4/5) — so ~a third of the excess is right-tail. 18-yr compounding: 26x to 145x depending on tail treatment. Part VIII, Result 43 |
| "Buy the low, sell the peak" as a rule | ❌ **REFUTED across all 654 IDX names.** The optimal trades genuinely sit at the 3rd/90th percentile of RSI and are held 220-550 days (Result 40) — but identifying them ex ante does not work: 4.4x vs 5.9x for owning the universe, median trade -0.6%, and stops make it worse. Part X |
| Any spectacular IDX backtest number | ⚠️ **Check for corrupt daily returns first.** 0.044% of daily returns in the IDX panel are physically impossible (KOPI prints +10,915,284% on zero turnover). Uncapped, they turned a 4.4x strategy into a 195x one and the benchmark into 34,589,778x. Result 41 |
